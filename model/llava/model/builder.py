...
# Updated on Nov 25, 2024, based on the https://github.com/LLaVA-VL/LLaVA-NeXT
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or is_multimodal or "flame" in model_name.lower() :
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            print(":::::::::::::::::::::model_base --NOT NONE-- MODE:::::::::::::::::::::")
            ...
            elif "flame" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_base)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = FlameDeepseekForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            elif (
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    print("\t**DEBUG**: qwen_NON_moe")
                    from llava.model.language_model.llava_qwen import LlavaQwenConfig
                    if overwrite_config is not None:
                        print("\t**DEBUG**: qwen_NON_moe_Overwrite")
                        ...
                        print("\t**DEBUG**: qwen_NON_moe_NON_Overwrite")
                        model = LlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
                        print("\t**DEBUG**: qwen_NON_moe_NON_Overwrite::::DONE")
                ...
                # tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                ### flame config 有问题，需要查一下，  attn_implementation=attn_implementation,需要安装新的包
                cfg_pretrained = FlameConfig.from_pretrained(model_path)
                # model = FlameDeepseekForCausalLM.from_pretrained(
                #     model_path, low_cpu_mem_usage=True,config=cfg_pretrained,**kwargs,
                # )
                model = FlameDeepseekForCausalLM.from_pretrained(model_path, attn_implementation=attn_implementation, 
                low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
            else:
                try:
                    from llava.model.language_model.llava_llama import LlavaConfig
    ...
    # print('returning tokenizer: ', is_multimodal, image_processor)
