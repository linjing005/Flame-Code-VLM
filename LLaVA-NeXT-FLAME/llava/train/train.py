...
#    Modified for FLAME:A large vision language model for front-end development (Dec, 2024)
#    See original code: [https://github.com/LLaVA-VL/LLaVA-NeXT]
...
class DataArguments:
    ...
    sft_tasks: Optional[str] = field(default='v1')
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # device: str = field(default="cuda", metadata={"help": "training devices"})
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    # len(trainer.args.mm_tunable_parts.split(",")) == 1 and
...
# preprocess the source prompt+input+output and return the input_ids and target_ids
def preprocess_flame(
    ...
    # prompt_temp = PROMPT_DICT["prompt_layout_input"]
    # Concat instucitons + solutions, This version we only consider one-round instruction+solution
    # For the front-end coding, we have instruction, layout desc, code:css, code:js/ts
    # We may have the following combinations:
    ...
    task_pool = sft_tasks.split("_")
    task_num = len(task_pool)
    if task_num == 1:
        ver_idx = 0
    elif task_num > 1:
        ver_idx = random.randint(0, task_num-1) 
    else:
        ValueError(f"Task_num SHOULD BE >= 1")

    prefix_tplt = PROMPT_DICT[task_pool[ver_idx]]
    resp_tplt = PROMPT_DICT[task_pool[ver_idx]+'_res']
    prefix = prefix_tplt.format_map(dict(instruction=source_example['instruction_requirement'].replace(DEFAULT_IMAGE_TOKEN,""), layout=source_example['instruction_layout'], css_code=source_example['style'], code=source_example['component'], image=DEFAULT_IMAGE_TOKEN))
    resp = resp_tplt.format_map(dict(instruction=source_example['instruction_requirement'].replace(DEFAULT_IMAGE_TOKEN,""), layout=source_example['instruction_layout'], css_code=source_example['style'], code=source_example['component'], image=DEFAULT_IMAGE_TOKEN))

    example = prefix + resp + tokenizer.eos_token

    # Tokenize the prompt; if the data contains an image, then mark the image_index in the tokenized input_ids 
    if has_image:  
        prefix_len = len(tokenizer_image_token(prefix, tokenizer, return_tensors='pt')) 
        input_ids = tokenizer_image_token(example, tokenizer, return_tensors='pt') 
    else:
    ...
    targets = copy.deepcopy(input_ids)
    targets[:prefix_len] = IGNORE_INDEX
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    if conversation_lib.default_conversation.version == "deepseek_coder":
        return preprocess_flame(sources, tokenizer, has_image=has_image)
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if source_example['image'] != '':
            ...
            if type(image_file) is list:
                ...
                image = [self.process_image(image_file)] 
                # rank0_print(f"***DEBUG***{image_file} \n{len(image)}\n")

        source_example = copy.deepcopy(source_example)
        has_image = source_example['image'] != ''
        data_dict = preprocess_flame(source_example, self.tokenizer, self.data_args.sft_tasks, has_image=has_image)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        # rank0_print(f"type(instances) : {type(instances)} ||| instances.length : {len(instances)} ||| instances[0] : {instances[0]}")
        input_ids, labels = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels"))
        # input_ids, labels, ids = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels", "id"))
        # rank0_print(f"type(input_ids) : {type(input_ids)} ||| input_ids[0] : {input_ids[0]}")
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    if "gh_star10_variation" in data_args.data_path or "flame_ft" in data_args.data_path :
        train_dataset = LazySupervisedMMCoderDataset(tokenizer=tokenizer, data_path=data_args.data_path, data_args=data_args)
    else:
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        if model_args.model_class_name == "tunable":
            model = FlameDeepseekForCausalLM.from_pretrained(
        ...
        if "deepseek" in model_args.model_name_or_path.lower():
            rank0_print("loading flamedeepseekforcausalLM ...") 
            model = FlameDeepseekForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
...
def train(attn_implementation=None):
    ...
    # rank0_print(f"{training_args}")
    # rank0_print(f"There are {len(data_args.sft_tasks.split('_'))} recipe tasks {data_args.sft_tasks}...\n")
    ...
    if "mistral" in model_args.model_name_or_path.lower() or "mixtral" in model_args.model_name_or_path.lower() or "zephyr" in model_args.model_name_or_path.lower():
        ...
        or "yi" in model_args.model_name_or_path.lower()
        or "nous-hermes" in model_args.model_name_or_path.lower()
        or "wizard-2" in model_args.model_name_or_path.lower()
        or "deepseek" in model_args.model_name_or_path.lower()
    ):
        rank0_print("*******DEBUG***********:or deepseek in model_args.model_name_or_path.lower()\n")
        if os.path.isdir(model_args.model_name_or_path):
            rank0_print(f"Contents of {model_args.model_name_or_path}:")
            rank0_print(os.listdir(model_args.model_name_or_path))
        else:
            rank0_print(f"{model_args.model_name_or_path} is not a directory")
        ...
        try:
            ...
            # 这里只捕获可能发生的、和 `use_fast=False` 相关的错误
            print(f"加载 tokenizer 时出错，错误信息: {e}，尝试改用另一种方式加载...")
            tokenizer = transformers.AutoTokenizer.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                model_max_length=training_args.model_max_length,
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if "mm_mlp_adapter" in tunable_parts:
                print("Make mm_mlp_adapter tunable!\n")
                for p in model.get_model().mm_projector.parameters():
                    p.requires_grad = True
            if "mm_vision_resampler" in tunable_parts:
                print("Make mm_vision_resampler tunable!\n")
                for p in model.get_model().vision_resampler.parameters():
                    p.requires_grad = True
            if "mm_vision_tower" in tunable_parts:
                print("Make mm_vision_tower tunable!\n")
                flag_debug=0
                for name, param in model.named_parameters():
                    if flag_debug < 3:
                        print(f"---TOWER--- Tunable {name}\n")
                        flag_debug+=1
                    if "vision_tower" in name:
                        param.requires_grad_(True)
            if "mm_language_model" in tunable_parts:
                print("Make mm_language_model tunable!\n")
                flag_debug=0
                for name, param in model.named_parameters():
                    if flag_debug < 3:
                        print(f"---LLM--- Tunable {name}\n")
                        flag_debug+=1
